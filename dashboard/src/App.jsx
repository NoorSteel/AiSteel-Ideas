...
function isMeaninglessMessage(content) {
  ...
  // 2. Filler website references or bot statistics (e.g. AiSteel.it.com 55, AiSteel.ae 125, etc.)
  if (/aisteel\s*\.\s*[a-z0-9]+([\.-][a-z0-9]+)*/i.test(contentNorm)) {
    return true;
  }
  ...
  const normalizedContent = contentNorm.replace(/[^\w\s\u0600-\u06FF]/g, ' ');
  const words = normalizedContent.split(' ').filter(w => w.length > 0);
  ...
  const hasSteelKeyword = steelKeywords.some(kw => contentNorm.includes(kw));
  
  const alphanumericOnly = contentNorm.replace(/[^\w\u0600-\u06FF]/g, '').trim();
